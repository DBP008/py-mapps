...
def _():
    ...
    def get_gamma_phasor(shg_intensity_full, theta_L_full_rad, theta_F_pred_rad):
        ...
        return g, s
    return get_gamma_phasor, get_theta_phasor
...
def _(get_gamma_phasor, get_theta_phasor):
    ...
    def generate_gamma_ref_curve(
        ...
        return ref_tuples
    return generate_gamma_ref_curve, generate_theta_ref_curve
...
def _(
    ...
    df_theta_ref = pl.DataFrame(
        theta_ref_tuples, schema=["theta_F_val_rad", "g", "s"]
    )
    ...
    df_gamma_ref = pl.DataFrame(gamma_ref_tuples, schema=["gamma", "g", "s"])
    ...
    gamma_pred_vec_val = extract_gamma_vectorized(
        g_ga_exp, s_ga_exp, gamma_ref_tuples
    )
    gamma_pred_vec = (
        gamma_pred_vec_val.item()
        if isinstance(gamma_pred_vec_val, np.ndarray)
        and gamma_pred_vec_val.ndim == 0
        else gamma_pred_vec_val
    )
